...
export default function AddTasks(props:any) {
  ...
  function validateInput(task:string):boolean {
    return (task.length<32 && task.length>0 && task!== "")?true:false;
  }
  return (
    <div class="container mx-auto mt-2">
      <form class="my-6" autocomplete="off" onSubmit={onSubmit}>
        <input type="text" name="addTask" class="mx-2 rounded-full text-black text-center border border-transparent focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent" id="addTask" placeholder="Add a task" onInput={(e:any) => addInputChange(e.target.value)} />
